...
function registerRecordingExtras({
  electron, preload, rendererDir, userDataPath, excludeFromCapture, now,
  togglePause, refreshBar
}) {
  const { ipcMain, BrowserWindow, globalShortcut, systemPreferences } = electron;
  const settingsFile = () => path.join(userDataPath(), 'recording.json');
  let settings = null;
  // Like settings.js, a settings problem must never stop a recording: with no
  // readable file (or no userData path yet) the defaults apply.
  const current = () => {
    if (!settings) {
      try {
        settings = loadRecordingSettings(settingsFile());
      } catch {
        return loadRecordingSettings(null);
      }
    }
    return settings;
  };
  ...
  ipcMain.handle('recordingSettings:set', (_e, patch) => {
    settings = saveRecordingSettings(settingsFile(), applyRecordingSettingsPatch(current(), patch));
    return settings;
  });
